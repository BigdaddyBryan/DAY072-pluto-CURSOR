...
$theme = 'dark';
$sourceCssPath = __DIR__ . '/../../custom/custom/css/custom-dark.css';
$publicCssPath = __DIR__ . '/../../public/custom/css/custom-dark.css';
$schema = customCssEditorTokenSchema();
$defaults = customCssEditorDefaultValues($theme);
$darkCss = customCssEditorReadContent($sourceCssPath, $publicCssPath);
...
<form method="POST" action="/custom-css-editor?comp=dark" id="customCssEditorFormDark" class="customCssEditorForm"
  data-custom-css-editor="true" data-theme="dark" novalidate>
  <div class="customCssEditorHeader">
    <div class="customCssEditorTitleRow">
      <h2 class="customCssEditorTitle"><?= htmlspecialchars(customCssEditorUiText('admin.edit_dark_css', 'Edit Dark CSS'), ENT_QUOTES, 'UTF-8'); ?></h2>
      <span class="customCssEditorThemeBadge"><?= htmlspecialchars(customCssEditorUiText('admin.dark_theme', 'Dark Theme'), ENT_QUOTES, 'UTF-8'); ?></span>
    </div>
      ...
      <section class="customCssTokenSection" data-token-category="<?= $groupId; ?>">
        ...
        <div class="customCssTokenGrid">
          <?php foreach (($group['entries'] ?? []) as $entry): ?>
            ...
            $entryId = 'dark-css-token-' . (string) $entry['name'];
